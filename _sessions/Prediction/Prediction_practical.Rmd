...
5. You should already be familiar with the dataset, but you can refresh your memory by checking the variable names with `names()` and the contents using `glimpse()`.
...
26. Using the `predict()` function, to extract the model predictions from `price_rf` and bind them together with the true values from `airbnb_train` using `bind_cols()`.
...
27. Using the `metrics()` function, evaluate the model performance. Pass it the `price` variable as `truth` and the `.pred` variable as `estimate`.
...
28. How does the training performance of the random forest compare to the ones of the other two models?


29. Using the following code, plot the fitted against the true value, to judge how well our model performed.
...
```{r}
# The random forest has the lowest errors, but the regression model has the
# highest R2 values.
```
...
1. Let's again turn to a classification example. We will again focus on the `host_superhost` variable. Like in the previous practical, we first have to change our criterion to be a `factor`. We again explicitly specify `"yes"` as first level.
...
```{r, echo = TRUE, eval = FALSE}
airbnb_split <- initial_split(XX, prop = XX)
```

```{r}
airbnb_split <- initial_split(airbnb, prop = .8)
```
...
8. Let's look at different performance metrics. Use the `metrics()` function and pass it the `host_superhost` variable as `truth`, the `.pred_class` variable as `estimate`, and `.pred_yes` as last argument.
...
16. Let's look at different performance metrics. Use the `metrics()` function and pass it the `host_superhost` variable as `truth`, the `.pred_class` variable as `estimate`, and `.pred_yes` as last argument.


```{r}
metrics(dt_pred, truth = host_superhost, estimate = .pred_class, .pred_yes)
```
...
```{r}
roc_curve(dt_pred, truth = host_superhost, .pred_yes) %>% 
  autoplot()
```
...
23. Let's look at different performance metrics. Use the `metrics()` function and pass it the `host_superhost` variable as `truth`, the `.pred_class` variable as `estimate`, and `.pred_yes` as last argument.


```{r}
metrics(rf_pred, truth = host_superhost, estimate = .pred_class, .pred_yes)
```
...
```{r}
roc_curve(rf_pred, truth = host_superhost, .pred_yes) %>% 
  autoplot()
```
...
```{r}
# evaluate performance
metrics(glm_pred_test, truth = host_superhost, estimate = .pred_class, .pred_yes)
metrics(dt_pred_test, truth = host_superhost, estimate = .pred_class, .pred_yes)
metrics(rf_pred_test, truth = host_superhost, estimate = .pred_class, .pred_yes)
```

4. Which model performs the best based on the test data? 

```{r}
# The random forest predictions are the most accurate.
```

5. Plot the ROC-curves of the test performances.
```{r}
roc_curve(glm_pred_test, truth = host_superhost, .pred_yes) %>% 
  autoplot()
roc_curve(dt_pred_test, truth = host_superhost, .pred_yes) %>% 
  autoplot()
roc_curve(rf_pred_test, truth = host_superhost, .pred_yes) %>% 
  autoplot()
```
