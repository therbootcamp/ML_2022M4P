...
author: "<table style='table-layout:fixed;width:100%;border:0;padding:0;margin:0'><col width='10%'><col width='10%'>
  <tr style='border:none'>
    ...
    <font style='font-style:normal'>Applied Machine Learning with R</font><br>
      ...
      <a href='https://therbootcamp.github.io'>
        <font style='font-style:normal'>The R Bootcamp @ Methods 4 Postocs</font>
      </a>
    </td>
...
<img src="image/fitting_dirk.001.png" width="100%" margin="0"/><br> <font style="font-size:10px">adapted from [xkcd.com](https://xkcd.com/)</font>
...
5.  Take a look at the data using `glimpse()`.
...
6.  Open the dataset in a new window using `View()`. How does it look?

```{r, echo = TRUE, eval = FALSE}
View(XX)
```

7.  Familiarize yourself with the names of the dataset by looking at the feature names using `names()`.
...
1.  By specifying a `recipe`, we specify (a) what to predict, (b) how to predict it (the features), and (c) how to prepare our data. Create your first recipe called `airbnb_recipe`, by adding a formula to specify that we want to predict `price` with the number of people it can accommodate (`accommodates`):
...
1.  Now we can finally specify our model `workflow` in which we bring the model specification and the recipe together, to then fit our model. To do so
...
3.  Now it's time to actually fit the model using the `fit()` function. Pass the `lm_workflow` into the fit function and save it as `price_lm`. Also we have to provide the data to the fit function, by specifying the `data` argument. Set `data = airbnb`.
...
```{r}
# On average, the model commits a prediction error of 23.8 when predicting the 
...
# The R^2 value is 0.451, that is, about 45% of the variation in the data
...
```{r}
# On average, the model commits a prediction error of 22.9 when predicting the 
# price of an airbnb. This is already a bit lower than the one with just one predictor.
# The large difference between the MAE and the RMSE indicates
# that the prediction errors vary very strongly. This is also apparent in the 
# plot we created before.
# The R^2 value is 0.517, that is, about 52% of the variation in the data
# can be captured by our model, which is about 1.5 times of what the model with
...
```{r}
# With all predictors, the model still has some issues, but overall, seems to do
# a decent job.
```
...
```{r}
# On average, the model commits a prediction error of 21.0 when predicting the 
# price of an airbnb. This is again a bit lower than the one with four predictors.
# The large difference between the MAE and the RMSE indicates
# that the prediction errors vary very strongly. This is also apparent in the 
# plot we created before.
# The R^2 value is 0.586, that is, about 59% of the variation in the data
# can be captured by our model.
```
...
2.  The `host_superhost` variable is of class `character`. Therefore, we have to change it to `factor`. **Important note**: In binary classification tasks, the **first** factor level will be chosen as positive. We therefore explicitly specify, that `yes` be the first level.

```{r, echo = TRUE}
# Recode host_superhost to be a factor with "yes" as first level
airbnb <-
  airbnb %>% 
  mutate(host_superhost = factor(host_superhost, levels = c("yes", "no")))
```

3.  Check again, whether `host_superhost` is now a factor, and check whether the order of the levels is as intended using `levels()` (the order should be `"yes", "no"`).
...
2.  Take a look at the `logistic_pred` object and think about what the variables mean. If you're unsure, discuss with your neighbor or ask =)
...
# The third variable, .pred_class, contains the predicted class. If the .pred_yes
# variable in a given row was >=.5, this will be TRUE, otherwise it will be FALSE.
# Finally, the last variable, host_superhost, contains the actual values.
```

3.  Now, get the confusion matrix using the `conf_mat()` function and passing it the `host_superhost` variable as `truth`, and the `.pred_class` variable as `estimate`. Just by looking at the confusion matrix, do you think the model is doing well?
...
```{r}
metrics(logistic_pred, truth = host_superhost, estimate = .pred_class, .pred_yes)
```
...
# sensitivity and specificity simultaneously take on perfect values.
```

8.  To plot the ROC curve, we can use the `roc_curve()` function, to create sensitivity and specificity values of different cut-offs, and pass this into the `autoplot()` function, to plot the curve. Add the `host_superhost` column as `truth`, and the `.pred_yes` column as third, unnamed argument, to the `roc_curve()` function and plot the curve.
...
```{r}
roc_curve(logistic_pred, truth = host_superhost, .pred_yes) %>% 
  autoplot()
```
...
The dataset contains data of 500 apartments that were added on Airbnb for the Berlin area in the year 2018.

| File                                                                                                          | Rows | Columns |
|:-------------------|:-----------------------|:---------------------------|
| [airbnb.csv](https://raw.githubusercontent.com/therbootcamp/ML_2022M4P/master/TheRBootcamp/1_data/airbnb.csv) | 500  | 23      |
...
| host_superhost              | Whether host is a superhost "yes"/"no"                    |
...
| kitchen                     | Kitchen available "yes"/"no"                              |
| tv                          | TV available "yes"/"no"                                   |
| coffe_machine               | Coffee machine available "yes"/"no"                       |
| dishwasher                  | Dishwasher available "yes"/"no"                           |
| terrace                     | Terrace/balcony available "yes"/"no"                      |
| bathtub                     | Bathtub available "yes"/"no"                              |
