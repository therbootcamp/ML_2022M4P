...
author: "<table style='table-layout:fixed;width:100%;border:0;padding:0;margin:0'><col width='10%'><col width='10%'>
  <tr style='border:none'>
    <td style='display:block;width:100%;text-align:left;vertical-align:bottom;padding:0;margin:0;border:none' nowrap>
      ...
      <a href='https://therbootcamp.github.io'>
        <font style='font-style:normal'>The R Bootcamp @ Methods 4 Postdocs</font>
      </a>
    </td>
...
1. Create an initial split of the data, where you allocate only 80% of the data to the training set. Save it as `airbnb_split`

```{r}
airbnb_split <- initial_split(airbnb, prop = .80)
```
...
9. Hm, the tuning did not seem to have a large effect. But the maximum penalty value automatically chosen by the `grid_regular` function was only 1. Specify your own grid of parameter values with the code below and then repeat steps 5 to 7 from above.
...
14. The `last_fit()` function, which you can use with the `final_ridge` workflow instead of the `fit()` function, let's you directly evaluate the model performance. Pass the `final_ridge` and `airbnb_split` (! instead of the `airbnb_train`) objects into `last_fit()` and directly pipe the output into `collect_metrics()` (this is just to showcase this possibilty).
...
Now it's time to fit a lasso regression. We could use the `ridge_recipe` from before, but given that Lasso can perform automatic feature selection and thus handle a large number of predictors well, let's expand the `ridge_recipe` by adding a step that will add all possible two-way interaction terms.

1. To add the interaction terms of the to the recipe, use the `ridge_recipe` and add to it a `step_interact()` with the argument `terms = price ~ (.)^2`. The `(.)^2` is a short-cut, to add all two-way interactions to the model.

```{r, eval = FALSE, echo = TRUE}
# create lasso recipe by adding step_interact() to ridge_recipe
lasso_recipe <- 
  XX %>% 
  XX(XX = XX)
...
lasso_recipe <- 
  ridge_recipe %>% 
  step_interact(terms = price ~ (.)^2)
  
```

2. Create a lasso regression model, again using the `linear_reg()` model and the  `glmnet` engine. Within `linear_reg()`, set the argument `mixture` to 1 and `penalty` to `tune()`. Save the output as `lasso_model`
...
3. Create a workflow called `lasso_workflow` using `workflow()` and add the `lasso_recipe` and `lasso_model` objects.
...
4. Set up a grid of tuning parameters. Here we will directly specify our own grid. We'll use the same as with the ridge regression:

```{r, eval = FALSE, echo = TRUE}
penalty_grid <- tibble(penalty = seq(0, 25, length.out = 100))
```
...
5. Tune the model on the training data. To do so using our resampling scheme, use the `tune_grid()` function. Pass it the defined workflow as first argument and set the `resamples` argument to `airbnb_folds`, and the `grid` argument to `penalty_grid`. Save the output as `lasso_grid`.
...
7. Plot the `penalty` value on the x axis, and the performance metrics on the y axis. Use the code below to do so.
...
8. Select the best `penalty` value by passing `lasso_grid` into the `select_best()` function and save the output as `best_lasso`.
...
9. Finalize the workflow using the `finalize_workflow()` function. Pass it the `lasso_workflow` and the `best_lasso` objects as first and second arguments. Save the output as `final_lasso`.
...
10. Fit the model using the final workflow. Use the `fit()` function to this end, by passing it the `final_lasso` workflow and `airbnb_train`. Save the output as `lasso_res`.

```{r}
lasso_res <- fit(final_lasso, airbnb_train) 
```

11. Use the `tidy()` function to look at the parameter values of `lasso_res`. Which variables are important and which were set to 0?
...
5. Tune the model on the training data. To do so using our resampling scheme, use the `tune_grid()` function and pass it the defined workflow as first argument and set the `resamples` argument to `airbnb_folds`, and the `grid` argument to `complexity_grid`. Save the output as `dt_grid`. (Note: This may take some time...)
...
4. Tune the model on the training data. To do so using our resampling scheme, use the `tune_grid()` function and pass it the defined workflow as first argument and set the `resamples` argument to `airbnb_folds`, and the `grid` argument to `mtry_grid`. Save the output as `rf_grid`. (Note: This will again take some time...)
...
2. Which of your models had the best performance in the test data?

3. How close were your models' test-prediction error to the values you estimated in the previous section based on the training data?
